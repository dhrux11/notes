...
?>
      ...
      <input type="button" class="button button4" value="Accept" name="accept">
        <input type="button" class="button button4" value="Reject" name="reject">
        <a href="CasStudy.pdf" target="_blank"><button class="button button4">View</button></a></form>
       </center>
    <?php 
    if($_REQUEST['do']=="showform"){
///// Show insert form   
echo "<form action=\"index_php.php\" method=\"get\" enctype=\"text/plain\">
...
if($_REQUEST['do']=="insert"){
    ...
    $name=mysql_real_escape_string($_GET['Accept']);
    /// call your insert function here 
    /// for Example

  $sql=$db->query("UPDATE  uploaded_notes SET topic=1 WHERE topic=0 LIMIT 1;");

  if($sql){///  
   echo "<META HTTP-EQUIV='refresh' CONTENT='1; URL=index_php.php?do=showform'>"; 
  } 
  else{
